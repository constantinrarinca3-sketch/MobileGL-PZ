...
fail() { printf 'VERIFY_022=FAIL reason=%s\n' "$*" >&2; exit 1; }
need_source() {
...
need_source 'Perf022A = 1u << 24' 'MobileGL/PZOptLab.h'
need_source 'Perf022B = 1u << 25' 'MobileGL/PZOptLab.h'
need_source 'Perf022C = 1u << 26' 'MobileGL/PZOptLab.h'
need_source 'Perf022D = 1u << 27' 'MobileGL/PZOptLab.h'
need_source 'Perf022E = 1u << 28' 'MobileGL/PZOptLab.h'
...
need_source 'MOBILEGL_PZ_OPT_022A' 'MobileGL/PZOptLab.cpp'
need_source 'MOBILEGL_PZ_OPT_022B' 'MobileGL/PZOptLab.cpp'
need_source 'MOBILEGL_PZ_OPT_022C' 'MobileGL/PZOptLab.cpp'
need_source 'MOBILEGL_PZ_OPT_022D' 'MobileGL/PZOptLab.cpp'
need_source 'MOBILEGL_PZ_OPT_022E' 'MobileGL/PZOptLab.cpp'
need_source 'schema=5 build=OPT-LAB-V3-022' 'MobileGL/PZOptLab.cpp'
need_source 'ProcessTexturePixelsDataUnpackInto' 'MobileGL/MG_Util/Texture/PixelStoreProcessor.cpp'
need_source 'Optimization::Perf022A' 'MobileGL/MG_Impl/GLImpl/Texture/GL_Texture.cpp'
need_source 'Optimization::Perf022B' 'MobileGL/MG_Impl/GLImpl/Texture/GL_Texture.cpp'
need_source 'SetAutoGenerateMipmapEnabled' 'MobileGL/MG_Impl/GLImpl/Texture/GL_Texture.cpp'
need_source 'containedByExistingUnion' 'MobileGL/MG_State/GLState/TextureState/MipmapStorage.cpp'
need_source 'markDirtyAfterDsaSubImage' 'MobileGL/MG_Impl/GLImpl/Texture/GL_Texture.cpp'
...
if [[ -n "$ELF" ]]; then
    ...
    strings "$ELF" | grep -Fq 'OPT-LAB-V3-022' || fail 'elf-marker-missing'
    strings "$ELF" | grep -Fq 'MOBILEGL_PZ_OPT_022E' || fail 'elf-selector-missing'
fi

printf 'VERIFY_022=PASS source=%s%s\n' "$SOURCE_ROOT" \
