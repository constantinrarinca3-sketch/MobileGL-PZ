    ...
    namespace MG_State {
        namespace GLState {
            ...
            public:
                ...
                // Compatibility glPushClientAttrib/glPopClientAttrib snapshots the
                // complete vertex-array client state, not only the currently bound
                // VAO name. Restore that state into this live VAO while preserving
                // its GL name and lifetime identity. Returns true when any state
                // actually changed.
                Bool RestoreClientAttribStateFrom(const VertexArrayObject& source);
