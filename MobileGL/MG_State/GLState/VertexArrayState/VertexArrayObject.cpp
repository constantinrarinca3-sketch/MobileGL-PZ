...
namespace MobileGL::MG_State::GLState {
    ...
    Bool VertexArrayObject::RestoreClientAttribStateFrom(const VertexArrayObject& source) {
        Bool changed = m_indexBufferBindingSlot.GetBoundObject() !=
                       source.m_indexBufferBindingSlot.GetBoundObject();

        for (Uint index = 0; index < MAX_VERTEX_ATTRIBS; ++index) {
            const auto& active = m_attributes[index];
            const auto& saved = source.m_attributes[index];
            const auto& activeBinding = m_bindingPoints[index];
            const auto& savedBinding = source.m_bindingPoints[index];
            changed = changed || active.Enabled != saved.Enabled || active.Size != saved.Size ||
                      active.Type != saved.Type || active.Normalized != saved.Normalized ||
                      active.Stride != saved.Stride || active.Offset != saved.Offset ||
                      active.IsInteger != saved.IsInteger || active.IsLong != saved.IsLong ||
                      active.IsBgra != saved.IsBgra || active.Divisor != saved.Divisor ||
                      active.Buffer != saved.Buffer || active.LegacyStride != saved.LegacyStride ||
                      active.LegacyPointer != saved.LegacyPointer ||
                      activeBinding.Buffer != savedBinding.Buffer ||
                      activeBinding.Offset != savedBinding.Offset ||
                      activeBinding.Stride != savedBinding.Stride ||
                      activeBinding.Divisor != savedBinding.Divisor ||
                      m_attributeBindingIndex[index] != source.m_attributeBindingIndex[index] ||
                      m_attributeRelativeOffset[index] != source.m_attributeRelativeOffset[index] ||
                      m_attributeUsesBindingModel[index] != source.m_attributeUsesBindingModel[index];
        }
        if (!changed) return false;

        m_attributes = source.m_attributes;
        m_indexBufferBindingSlot = source.m_indexBufferBindingSlot;
        m_bindingPoints = source.m_bindingPoints;
        m_attributeBindingIndex = source.m_attributeBindingIndex;
        m_attributeRelativeOffset = source.m_attributeRelativeOffset;
        m_attributeUsesBindingModel = source.m_attributeUsesBindingModel;

        // The live object may already be memoized by either backend. Keep its
        // identity, but make every vertex-input consumer observe the restore.
        for (auto& version : m_attributeVersions) {
            ++version.FormatVersion;
            ++version.BufferVersion;
            ++version.SwitchVersion;
        }
        ++m_configVersion;
        m_backendHashMemoVersion = ~0u;
        m_backendStateMemoVersion = ~0u;
        m_backendAuxMemoVersion = ~0u;
        return true;
    }
