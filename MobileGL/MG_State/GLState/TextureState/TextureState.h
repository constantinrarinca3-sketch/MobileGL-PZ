...
namespace MobileGL::MG_State::GLState {
    ...
    public:
        ...
        // parameter setters with existing identical-value guards do not churn it on redundant
        // glTexParameteri calls. Setters without such a guard conservatively invalidate. Kept separate
        ...
        // Exact coarse invalidation for the three DirectGLES draw-sync probes:
        // texture parameters and CPU-side texel content bump this counter. Sampler
        // changes remain covered by m_samplingResolutionGeneration. A backend can
        // therefore skip the per-texture version probes only while both counters
        // match a snapshot that was verified clean.
        Uint64 GetDrawSyncGeneration() const { return m_drawSyncGeneration; }
        void BumpDrawSyncGeneration() { ++m_drawSyncGeneration; }
