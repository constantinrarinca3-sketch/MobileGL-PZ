    ...
    namespace MG_State {
        namespace GLState {
            namespace {
                ...
                Bool RegionContains(const MipmapDirtyRegion& outer, const MipmapDirtyRegion& inner) {
                    return outer.lo.x() <= inner.lo.x() && outer.lo.y() <= inner.lo.y() &&
                           outer.lo.z() <= inner.lo.z() && outer.hi.x() >= inner.hi.x() &&
                           outer.hi.y() >= inner.hi.y() && outer.hi.z() >= inner.hi.z();
                }
            } // namespace
            ...
            void MipmapStorage::MarkDirtyRegion(Uint level, IntVec3 offset, IntVec3 size) {
                ...
                // 022D: an empty rect list means the union box is already the exact
                // upload contract. If a later write is wholly inside that box, seeding
                // a 96-slot vector and cascade-merging it back to the same union cannot
                // refine the upload at all. Keep the empty-list invariant and finish.
                const Bool perf022d = PZOptLab::Enabled(PZOptLab::Optimization::Perf022D);
                const Bool containedByExistingUnion =
                    m_isDirty[level] && level < m_dirtyRects.size() && m_dirtyRects[level].empty() &&
                    level < m_dirtyRegions.size() && !m_dirtyRegions[level].Empty() &&
                    RegionContains(m_dirtyRegions[level], incoming);
                if (perf022d) {
                    PZOptLab::RecordPath(PZOptLab::Optimization::Perf022D, 1,
                                         containedByExistingUnion ? 0 : 1);
                }
                if (perf022d && containedByExistingUnion) return;
