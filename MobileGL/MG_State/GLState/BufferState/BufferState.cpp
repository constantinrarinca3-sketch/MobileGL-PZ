    ...
    void BufferState::MarkBufferObjectForDeletion(Uint index) {
        if (m_indexGenerator.IsValid(index)) {
            ...
            if (it != m_bufferObjects.end()) {
                ...
                for (SizeT targetIndex = 0; targetIndex < m_bufferBindPointTargets.size(); ++targetIndex) {
                    auto& bindingPointArray = m_bufferBindPointTargets[targetIndex];
                    Bool changed = false;
                    ...
                    if (changed) {
                        auto& generation = m_bindPointGeneration[targetIndex];
                        ++generation;
