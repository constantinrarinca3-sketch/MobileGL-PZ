    ...
    namespace MG_Util {
        namespace ShaderTranspiler {
            ...
            inline bool IsLexicalPreemptRenameName(std::string_view name) {
                if (std::binary_search(std::begin(kLexicalPreemptRenameNames),
                                       std::end(kLexicalPreemptRenameNames), name)) {
                    return true;
                }
#if defined(MOBILEPZ_PZF1_PZ_MATH_BUILTIN_RENAME) || defined(MOBILEPZ_V1_CANDIDATE)
                // Project Zomboid's util/math helper family was captured on-device with
                // exact-signature precision-qualifier parse failures for max(), followed by
                // same-signature min()/clamp() declarations in the same source. Keep this
                // broadened lexical surface in the validated PZF1 fix and the clean V1 build.
                return name == "clamp" || name == "max" || name == "min";
#else
                return false;
#endif
            }
