...
namespace MobileGL::MG_Util::PixelStoreProcessor {
    ...
    namespace {
        struct UnpackWritePlan {
            Int width = 0;
            Int height = 0;
            Int depth = 0;
            Int effectiveHeight = 0;
            SizeT inputPixelSize = 0;
            SizeT inputRowStride = 0;
            SizeT outputPixelSize = 0;
            Bool packedDepthStencil = false;
            Bool floatDepthShadow = false;
            Bool needConversion = false;
            Bool isByteType = false;
            Bool isBitmap = false;
            UnpackConversionSpec conversion{};
        };

        Bool BuildUnpackWritePlan(const PixelStoreParameters& params, TextureInternalFormat targetInternalFormat,
                                  TextureInputFormat textureInputFormat, TexturePixelDataType inputDataType,
                                  IntVec3 dimension, Bool isBitmap, UnpackWritePlan& plan, SizeT& outSize) {
            plan.width = dimension.x();
            plan.height = dimension.y();
            plan.depth = dimension.z();
            plan.isBitmap = isBitmap;
            plan.inputPixelSize = MG_Util::GetInputBytesPerPixel(textureInputFormat, inputDataType);

            if (plan.width <= 0 || plan.height <= 0 || plan.depth <= 0 || plan.inputPixelSize == 0) {
                outSize = 0;
                return false;
            }

            const Int effectiveWidth = params.RowLength > 0 ? params.RowLength : plan.width;
            plan.effectiveHeight = params.ImageHeight > 0 ? params.ImageHeight : plan.height;
            plan.inputRowStride = CalculateRowStride(effectiveWidth, plan.inputPixelSize, params.Alignment);

            const Bool packedDepthStencilInternal =
                targetInternalFormat == TextureInternalFormat::Depth24Stencil8 ||
                targetInternalFormat == TextureInternalFormat::DepthStencil ||
                targetInternalFormat == TextureInternalFormat::Depth32FStencil8;
            plan.packedDepthStencil =
                !isBitmap && packedDepthStencilInternal &&
                textureInputFormat == TextureInputFormat::DepthComponent &&
                (inputDataType == TexturePixelDataType::Float ||
                 inputDataType == TexturePixelDataType::UnsignedInt ||
                 inputDataType == TexturePixelDataType::UnsignedShort);

            if (plan.packedDepthStencil) {
                plan.floatDepthShadow = targetInternalFormat == TextureInternalFormat::Depth32FStencil8;
                plan.outputPixelSize = plan.floatDepthShadow ? 8 : 4;
            } else {
                plan.needConversion = !isBitmap && GetUnpackConversionSpec(
                                                           targetInternalFormat, textureInputFormat, inputDataType,
                                                           plan.conversion);
                plan.outputPixelSize =
                    plan.needConversion ? plan.conversion.internalPixelSize : plan.inputPixelSize;
            }
            if (plan.outputPixelSize == 0 || plan.inputRowStride == 0) {
                outSize = 0;
                return false;
            }

            plan.isByteType = inputDataType == TexturePixelDataType::UnsignedByte ||
                              inputDataType == TexturePixelDataType::Byte;
            outSize = static_cast<SizeT>(plan.width) * static_cast<SizeT>(plan.height) *
                      static_cast<SizeT>(plan.depth) * plan.outputPixelSize;

            MGLOG_D("%s: start at: (%d, %d, %d), copy size: (%d, %d, %d), i/o row stride: (%d, %dx%d), "
                    "convert: %d",
                    __func__, params.SkipPixels, params.SkipRows, params.SkipImages, plan.width, plan.height,
                    plan.depth, static_cast<Int>(plan.inputRowStride), plan.width,
                    static_cast<Int>(plan.outputPixelSize), plan.needConversion ? 1 : 0);
            return outSize > 0;
        }

        Bool ExecuteUnpackWritePlan(const void* inputPixels, const PixelStoreParameters& params,
                                    TexturePixelDataType inputDataType, const UnpackWritePlan& plan,
                                    void* destination, SizeT destinationRowStride,
                                    SizeT destinationImageStride) {
            if (!inputPixels || !destination) return false;

            const SizeT rowBytes = static_cast<SizeT>(plan.width) * plan.outputPixelSize;
            if (destinationRowStride < rowBytes) return false;
            const SizeT imageSpan = static_cast<SizeT>(plan.height - 1) * destinationRowStride + rowBytes;
            if (plan.depth > 1 && destinationImageStride < imageSpan) return false;

            const Uint8* srcBase = static_cast<const Uint8*>(inputPixels) +
                static_cast<SizeT>(params.SkipImages) * static_cast<SizeT>(plan.effectiveHeight) *
                    plan.inputRowStride +
                static_cast<SizeT>(params.SkipRows) * plan.inputRowStride +
                static_cast<SizeT>(params.SkipPixels) * plan.inputPixelSize;
            auto* dstBase = static_cast<Uint8*>(destination);

            if (plan.packedDepthStencil) {
                for (Int z = 0; z < plan.depth; ++z) {
                    for (Int y = 0; y < plan.height; ++y) {
                        const Uint8* srcRow = srcBase +
                            static_cast<SizeT>(z) * static_cast<SizeT>(plan.effectiveHeight) * plan.inputRowStride +
                            static_cast<SizeT>(y) * plan.inputRowStride;
                        Uint8* dst = dstBase + static_cast<SizeT>(z) * destinationImageStride +
                                     static_cast<SizeT>(y) * destinationRowStride;
                        for (Int x = 0; x < plan.width; ++x) {
                            ...
                            if (plan.floatDepthShadow) {
                                const Uint32 stencilWord = 0;
                                Memcpy(dst, &depthValue, sizeof(depthValue));
                                Memcpy(dst + 4, &stencilWord, sizeof(stencilWord));
                                dst += 8;
                            } else {
                                const Uint32 depth24 = static_cast<Uint32>(std::llround(
                                    static_cast<double>(std::clamp(depthValue, 0.0f, 1.0f)) * 16777215.0));
            ...
            const Bool conversionSwapsBytes =
                plan.needConversion && params.SwapBytes && plan.conversion.swapGroupSize > 1;
            Vector<Uint8> swapScratch;
            if (conversionSwapsBytes) {
                swapScratch.resize(static_cast<SizeT>(plan.width) * plan.inputPixelSize);
            }

            for (Int z = 0; z < plan.depth; ++z) {
                const Uint8* layerSrc = srcBase +
                    static_cast<SizeT>(z) * static_cast<SizeT>(plan.effectiveHeight) * plan.inputRowStride;
                Uint8* layerDst = dstBase + static_cast<SizeT>(z) * destinationImageStride;

                for (Int y = 0; y < plan.height; ++y) {
                    Uint8* rowDst = layerDst + static_cast<SizeT>(y) * destinationRowStride;
                    if (plan.needConversion) {
                        const Uint8* rowSrc = layerSrc;
                        if (conversionSwapsBytes) {
                            Memcpy(swapScratch.data(), layerSrc,
                                   static_cast<SizeT>(plan.width) * plan.inputPixelSize);
                            const SizeT groupCount = static_cast<SizeT>(plan.width) * plan.inputPixelSize /
                                                     plan.conversion.swapGroupSize;
                            SwapBytes(swapScratch.data(), plan.conversion.swapGroupSize, groupCount);
                            rowSrc = swapScratch.data();
                        }
                        ConvertUnpackRow(rowSrc, rowDst, static_cast<SizeT>(plan.width), plan.conversion);
                    } else {
                        Memcpy(rowDst, layerSrc, static_cast<SizeT>(plan.width) * plan.inputPixelSize);

                        if (params.SwapBytes && !plan.isByteType) {
                            ...
                            if (swapGroup > 1) {
                                ...
                                SwapBytes(rowDst, swapGroup,
                                          static_cast<SizeT>(plan.width) * plan.inputPixelSize / swapGroup);
                            }
                        }

                        if (params.LSBFirst && plan.isBitmap) {
                            MGLOG_D("%s: LSBFirst", __func__);
                            ProcessLSBFirst(rowDst, static_cast<SizeT>(plan.width), 1);
                        }
                    }
                    layerSrc += plan.inputRowStride;
                }
            }
            return true;
        }
    ...
    void* ProcessTexturePixelsDataUnpack(const void* inputPixels, const PixelStoreParameters& params,
        ...
        UnpackWritePlan plan{};
        if (!BuildUnpackWritePlan(params, targetInternalFormat, textureInputFormat, inputDataType, dimension, isBitmap,
                                  plan, outSize)) {
            return nullptr;
        }
        ...
        const SizeT outputRowStride = static_cast<SizeT>(plan.width) * plan.outputPixelSize;
        const SizeT outputImageStride = static_cast<SizeT>(plan.height) * outputRowStride;
        if (!ExecuteUnpackWritePlan(inputPixels, params, inputDataType, plan, outputPixels, outputRowStride,
                                    outputImageStride)) {
            free(outputPixels);
            outSize = 0;
            return nullptr;
    ...
    Bool ProcessTexturePixelsDataUnpackInto(const void* inputPixels, const PixelStoreParameters& params,
                                            TextureInternalFormat targetInternalFormat,
                                            TextureInputFormat textureInputFormat,
                                            TexturePixelDataType inputDataType, IntVec3 dimension, Bool isBitmap,
        ...
        if (!BuildUnpackWritePlan(params, targetInternalFormat, textureInputFormat, inputDataType, dimension, isBitmap,
                                  plan, outSize) ||
            plan.outputPixelSize != destinationPixelSize) {
            outSize = 0;
            return false;
        }
        if (!ExecuteUnpackWritePlan(inputPixels, params, inputDataType, plan, destination,
                                    destinationRowStride, destinationImageStride)) {
            outSize = 0;
            return false;
        }
        return true;
