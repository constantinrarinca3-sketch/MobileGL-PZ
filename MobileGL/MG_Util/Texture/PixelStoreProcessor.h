...
namespace MobileGL::MG_Util::PixelStoreProcessor {
    ...
    // Same unpack/conversion contract as ProcessTexturePixelsDataUnpack, but writes
    // directly into caller-owned canonical shadow storage. `destination` points at
    // the first texel of the target region; row/image strides describe the complete
    // destination level. Returns false without writing when the canonical output
    // bytes-per-pixel does not match `destinationPixelSize` or the layout is invalid.
