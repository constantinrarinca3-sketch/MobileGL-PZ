...
namespace MobileGL::MG_Impl::GLImpl {
...
#ifdef MOBILEPZ_PZF13_NULL_TEXTURE_PRODUCER_LINEAGE
        void PZF13RecordFramebufferStatus(const MG_State::GLState::FramebufferObject& framebufferObject,
                                          GLenum status) {
            ...
            for (SizeT index = 0; index < attachments.size(); ++index) {
                ...
                if (!attachment.IsTexture() || !attachment.GetTexture()) continue;
                const auto& texture = attachment.GetTexture();
                ::MobilePZ::PZF13::RecordStatus(
                    texture->GetLifetimeId(), texture->GetExternalIndex(),
                    framebufferObject.GetExternalIndex(), static_cast<Int>(index), status,
    ...
    void InvalidateNamedFramebufferData_State(GLuint framebuffer, GLsizei numAttachments, const GLenum* attachments) {
        ...
        PZOptLab::RecordEvent(PZOptLab::Event::FboInvalidateCall,
                              static_cast<Uint64>(numAttachments));
        auto forward = MG_Backend::gBackendFunctionsTable.GL.InvalidateFramebufferData;
        if (PZOptLab::Enabled(PZOptLab::Optimization::Perf008) && forward) {
            forward(framebufferObject, GL_DRAW_FRAMEBUFFER, numAttachments, attachments);
        }
    }
    ...
    void InvalidateNamedFramebufferSubData_State(GLuint framebuffer, GLsizei numAttachments,
        ...
        if (!ValidateInvalidateSubRegion_State(width, height, "InvalidateNamedFramebufferSubData_State")) return;
        PZOptLab::RecordEvent(PZOptLab::Event::FboInvalidateCall,
                              static_cast<Uint64>(numAttachments));
        auto forward = MG_Backend::gBackendFunctionsTable.GL.InvalidateFramebufferSubData;
        if (PZOptLab::Enabled(PZOptLab::Optimization::Perf008) && forward) {
            forward(framebufferObject, GL_DRAW_FRAMEBUFFER, numAttachments, attachments,
                    x, y, width, height);
        }
    ...
    void InvalidateFramebuffer_State(GLenum target, GLsizei numAttachments, const GLenum* attachments) {
        ...
        auto forward = MG_Backend::gBackendFunctionsTable.GL.InvalidateFramebufferData;
        if (PZOptLab::Enabled(PZOptLab::Optimization::Perf008) && forward) {
            forward(framebufferObject, target == GL_FRAMEBUFFER ? GL_DRAW_FRAMEBUFFER : target,
    ...
    void InvalidateSubFramebuffer_State(GLenum target, GLsizei numAttachments, const GLenum* attachments, GLint x,
        ...
        if (!ValidateInvalidateSubRegion_State(width, height, "InvalidateSubFramebuffer_State")) return;
        PZOptLab::RecordEvent(PZOptLab::Event::FboInvalidateCall,
                              static_cast<Uint64>(numAttachments));
        auto forward = MG_Backend::gBackendFunctionsTable.GL.InvalidateFramebufferSubData;
        if (PZOptLab::Enabled(PZOptLab::Optimization::Perf008) && forward) {
    ...
    GLenum CheckFramebufferStatus_State(GLenum target) {
        ...
        if (!framebufferObject->CheckCompleteness()) {
            const GLenum status = HasDefinedAttachment(*framebufferObject) ?
                GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT :
                GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT;
#ifdef MOBILEPZ_PZF13_NULL_TEXTURE_PRODUCER_LINEAGE
            PZF13RecordFramebufferStatus(*framebufferObject, status);
#endif
            return status;
        }
        if (HasNonRenderableColorAttachment(*framebufferObject)) {
#ifdef MOBILEPZ_PZF13_NULL_TEXTURE_PRODUCER_LINEAGE
            PZF13RecordFramebufferStatus(*framebufferObject, GL_FRAMEBUFFER_UNSUPPORTED);
#endif
...
#ifdef MOBILEPZ_PZF13_NULL_TEXTURE_PRODUCER_LINEAGE
        PZF13RecordFramebufferStatus(*framebufferObject, GL_FRAMEBUFFER_COMPLETE);
#endif
    ...
    void ReadPixels(GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, void* pixels) {
        ...
        PZOptLab::ScopedEvent appReadbackMetric(
            PZOptLab::Event::AppReadback,
            static_cast<Uint64>(std::max(width, 0)) * static_cast<Uint64>(std::max(height, 0)));
