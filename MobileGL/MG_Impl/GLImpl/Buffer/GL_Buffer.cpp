...
namespace MobileGL::MG_Impl::GLImpl {
    ...
    static Bool IndexedBindingPointMatches(
        const BindingSlotRange1D<MG_State::GLState::BufferObject>& point,
        const SharedPtr<MG_State::GLState::BufferObject>& object,
        const Range1D& range, Bool hasExplicitRange) {
        if (point.GetBoundObject() != object || point.HasExplicitRange() != hasExplicitRange) return false;
        if (!hasExplicitRange) return true;
        const Range1D current = point.GetRange();
    ...
    void BindBufferBase_State(GLenum target, GLuint pointIndex, GLuint buffer) {
        ...
        SharedPtr<MG_State::GLState::BufferObject> bufferObject;
        if (buffer == 0) {
            const Bool unchanged = IndexedBindingPointMatches(point, bufferObject, Range1D(0, 0), true);
            point.Bind(nullptr);
            point.SetRange(Range1D(0, 0));
            if (!unchanged) MG_State::pGLContext->NoteBufferBindingPointChanged(bufferTarget);
        ...
        const Bool unchanged = IndexedBindingPointMatches(
            point, bufferObject, Range1D(0, bufferObject ? bufferObject->GetSize() : 0), false);
    ...
    void BindBufferRange_State(GLenum target, GLuint index, GLuint buffer, GLintptr offset, GLsizeiptr size) {
        ...
        const Range1D desiredRange(static_cast<SizeT>(offset), static_cast<SizeT>(offset + size));
        const Bool unchanged = IndexedBindingPointMatches(point, bufferObject, desiredRange, true);
        point.Bind(bufferObject);
        if (bufferObject) {
            point.SetRange(desiredRange);
        } else {
            point.ClearRange();
        }
        if (!unchanged) MG_State::pGLContext->NoteBufferBindingPointChanged(bufferTarget);
