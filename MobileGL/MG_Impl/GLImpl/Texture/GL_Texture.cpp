...
namespace MobileGL::MG_Impl::GLImpl {
    ...
    namespace {
        std::uint32_t ProofByteCount(SizeT bytes) {
            return static_cast<std::uint32_t>(
                std::min<SizeT>(bytes, static_cast<SizeT>(UINT32_MAX)));
        }

        Bool TryDirectTextureUnpack(PZOptLab::Optimization optimization, const void* inputPixels,
                                    const PixelStoreParameters& params,
                                    TextureInternalFormat targetInternalFormat,
                                    TextureInputFormat textureInputFormat,
                                    TexturePixelDataType inputDataType, IntVec3 dimension,
                                    void* destination, SizeT destinationPixelSize,
                                    SizeT destinationRowStride, SizeT destinationImageStride,
                                    SizeT logicalBytes, Bool instrumentationCompatible = true) {
            if (!PZOptLab::Enabled(optimization)) return false;

            SizeT directBytes = 0;
            const Bool success = instrumentationCompatible && destination &&
                MG_Util::PixelStoreProcessor::ProcessTexturePixelsDataUnpackInto(
                    inputPixels, params, targetInternalFormat, textureInputFormat,
                    inputDataType, dimension, false, destination, destinationPixelSize,
                    destinationRowStride, destinationImageStride, directBytes) &&
                directBytes == logicalBytes;
            const std::uint32_t proofBytes = ProofByteCount(logicalBytes);
            PZOptLab::RecordPath(optimization, proofBytes, success ? 0 : proofBytes, !success);
            return success;
        }
    ...
    namespace {
        ...
        // Writes the clear into the CPU shadow and marks only that texel box dirty, exactly like
        // TexSubImage*_State does. This matters when the rest of the level was written only on
        // the backend (FBO rendering, imageStore, CopyImageSubData): widening a partial clear to
        // a whole-level upload would overwrite those backend-written texels with stale shadow
        // bytes. A full-level clear naturally produces a whole-level dirty box.
        Bool ClearMipmapRegion(const SharedPtr<MG_State::GLState::TextureObjectMipmap>& textureObject,
            ...
            textureObject->MarkStorageDirtyRegion(uploadTarget, static_cast<Uint>(level),
                                                  {xoffset, yoffset, zoffset},
                                                  {width, height, depth});
            if (uploadTarget == TextureUploadTarget::Texture2DArray && depth == 1) {
                static std::atomic_flag markerLogged = ATOMIC_FLAG_INIT;
                if (!markerLogged.test_and_set(std::memory_order_relaxed)) {
                    MGLOG_I("PZCOMPAT_P21A_ARRAY_LAYER_DIRTY_REGION texture=%u level=%d layer=%d "
                            "size=%dx%d semantic_applied=1",
                            textureObject->GetExternalIndex(), level, zoffset, width, height);
                }
            }
            return true;
        }
...
#ifdef MOBILEPZ_PZF13_NULL_TEXTURE_PRODUCER_LINEAGE
        ::MobilePZ::PZF13::RecordWrite(textureObject->GetLifetimeId(), textureObject->GetExternalIndex(),
                                       0, -1, ::MobilePZ::PZF13::WriteKind::DirectClear);
#endif
    }
    ...
    void ClearTexSubImage(GLuint texture, GLint level, GLint xoffset, GLint yoffset, GLint zoffset,
        ...
        if (ClearMipmapRegion(textureObject, uploadTargets.front(), level, xoffset, yoffset, zoffset,
    ...
    void TextureParameterObject_State(const SharedPtr<MG_State::GLState::ITextureObject>& textureObject, GLenum pname,
        ...
        case GL_GENERATE_MIPMAP:
            if (PZOptLab::Enabled(PZOptLab::Optimization::Perf022C)) {
                textureObject->SetAutoGenerateMipmapEnabled(param != GL_FALSE);
            } else {
    ...
    void TextureParameterObjectf_State(const SharedPtr<MG_State::GLState::ITextureObject>& textureObject, GLenum pname,
        ...
        case GL_GENERATE_MIPMAP:
            if (PZOptLab::Enabled(PZOptLab::Optimization::Perf022C)) {
                textureObject->SetAutoGenerateMipmapEnabled(param != 0.0f);
            } else {
    ...
    void MaybeAutoGenerateMipmap(GLenum target, const SharedPtr<MG_State::GLState::ITextureObject>& textureObject,
        ...
        if (PZOptLab::Enabled(PZOptLab::Optimization::Perf022C)) {
            // The old path hashes the external texture name even for the overwhelmingly
            // common disabled case. A direct object latch is lifetime-correct and O(1).
            PZOptLab::RecordPath(PZOptLab::Optimization::Perf022C, 1, 0);
            if (!textureObject->IsAutoGenerateMipmapEnabled()) return;
        } else {
            const auto it = g_autoGenerateMipmapByTextureId.find(textureObject->GetExternalIndex());
            if (it == g_autoGenerateMipmapByTextureId.end() || !it->second) return;
        }
    ...
    void TexSubImage3D_State(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLint zoffset, GLsizei width,
        ...
        Uint8* directDestination =
            destData ? destData + static_cast<SizeT>(zoffset) * destSliceSize +
                           static_cast<SizeT>(yoffset) * destRowSize + static_cast<SizeT>(xoffset) * internalBpp
                     : nullptr;
        const SizeT logicalBytes = srcSliceSize * static_cast<SizeT>(depth);
        const auto& unpackParams = MG_State::pGLContext->GetPixelStoreParameters(true);
        if (TryDirectTextureUnpack(PZOptLab::Optimization::Perf022A, originalPixels, unpackParams,
                                   textureObject->GetFormat(), textureInputFormat, texturePixelDataType,
                                   {width, height, depth}, directDestination, internalBpp, destRowSize,
                                   destSliceSize, logicalBytes)) {
        ...
        void* processedPixels = MG_Util::PixelStoreProcessor::ProcessTexturePixelsDataUnpack(
            originalPixels, unpackParams, textureObject->GetFormat(), textureInputFormat, texturePixelDataType,
            {width, height, depth}, false, inputSize);
    ...
    void TexSubImage2D_State(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height,
...
#ifdef MOBILEPZ_V1_CANDIDATE
        if (level == textureObject->GetLevelRange().x()) {
            ::MobilePZ::V1::RecordDefinedAllocation(textureObject->GetLifetimeId());
        }
#endif
        const auto& unpackParams = MG_State::pGLContext->GetPixelStoreParameters(true);
        ...
        Uint8* destData = static_cast<Uint8*>(textureMipmapObject->MapMipmapData(textureUploadTarget, level));
        Uint8* directDestination =
            destData ? destData + static_cast<SizeT>(yoffset) * destRowSize +
                           static_cast<SizeT>(xoffset) * internalBpp
                     : nullptr;
        Bool directInstrumentationCompatible = true;
#ifdef MOBILEPZ_PZF9_MODEL_TEXTURE_UPLOAD_PROVENANCE
        // PZF9 hashes the contiguous processed buffer. Its diagnostic builds keep
        // the legacy path so the provenance capture remains byte-for-byte comparable.
        directInstrumentationCompatible = false;
#endif
        if (TryDirectTextureUnpack(PZOptLab::Optimization::Perf022A, originalPixels, unpackParams,
                                   textureInternalFormat, textureInputFormat, texturePixelDataType,
                                   {width, height, 1}, directDestination, internalBpp, destRowSize,
                                   static_cast<SizeT>(texelSize.y()) * destRowSize,
                                   srcRowSize * static_cast<SizeT>(height),
                                   directInstrumentationCompatible)) {
...
#ifdef MOBILEPZ_PZF9_MODEL_TEXTURE_UPLOAD_PROVENANCE
        const auto pzf9Source = ::MobilePZ::PZF9::AnalyzeUnpackSource(
            originalPixels, width, height, 1,
            MG_Util::GetInputBytesPerPixel(textureInputFormat, texturePixelDataType),
            unpackParams.Alignment, unpackParams.RowLength, unpackParams.ImageHeight,
            unpackParams.SkipPixels, unpackParams.SkipRows, unpackParams.SkipImages);
#endif
        void* processedPixels = MG_Util::PixelStoreProcessor::ProcessTexturePixelsDataUnpack(
...
#ifdef MOBILEPZ_PZF9_MODEL_TEXTURE_UPLOAD_PROVENANCE
        if (level == textureObject->GetLevelRange().x()) {
            ::MobilePZ::PZF9::RecordClientUpload(
                textureObject->GetLifetimeId(), textureObject->GetExternalIndex(),
                ::MobilePZ::PZF9::UploadKind::SubImage2D, level, xoffset, yoffset, width, height,
                static_cast<Uint>(textureInternalFormat), format, type, pixelUnpackBufferObject != nullptr,
                pzf9Source, ::MobilePZ::PZF9::AnalyzeBytes(processedPixels, inputSize));
        }
#endif
    ...
    void TexSubImage1D_State(GLenum target, GLint level, GLint xoffset, GLsizei width, GLenum format, GLenum type,
        ...
        Uint8* destData = static_cast<Uint8*>(textureMipmapObject->MapMipmapData(textureUploadTarget, level));
        Uint8* directDestination =
            destData ? destData + static_cast<SizeT>(xoffset) * internalBpp : nullptr;
        const auto& unpackParams = MG_State::pGLContext->GetPixelStoreParameters(true);
        if (TryDirectTextureUnpack(PZOptLab::Optimization::Perf022A, originalPixels, unpackParams,
                                   textureObject->GetFormat(), textureInputFormat, texturePixelDataType,
                                   {width, 1, 1}, directDestination, internalBpp, destRowSize,
                                   destRowSize, copySize)) {
            textureMipmapObject->MarkStorageDirtyRegion(textureUploadTarget, level, {xoffset, 0, 0},
                                                        {width, 1, 1});
            MaybeAutoGenerateMipmap(target, textureObject, false, level);
            return;
        }
    ...
    void TexImage3D_State(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height,
...
#ifdef MOBILEPZ_V1_CANDIDATE
        if (level == textureObject->GetLevelRange().x()) {
            // A real client/PBO definition supersedes an earlier NULL allocation.
            // Keep the production selector equivalent to PZF9's "last upload kind"
            // without retaining PZF9's byte hashing and per-upload telemetry.
            ::MobilePZ::V1::RecordDefinedAllocation(textureObject->GetLifetimeId());
        }
#endif
        ...
        const auto& unpackParams = MG_State::pGLContext->GetPixelStoreParameters(true);
        Uint8* directDestination =
            isProxy ? nullptr : static_cast<Uint8*>(textureMipmapObject->MapMipmapData(textureUploadTarget, level));
        const SizeT destinationRowStride = static_cast<SizeT>(width) * internalBpp;
        const SizeT destinationImageStride = static_cast<SizeT>(height) * destinationRowStride;
        if (!isProxy &&
            TryDirectTextureUnpack(PZOptLab::Optimization::Perf022B, originalPixels, unpackParams,
                                   textureInternalFormat, textureInputFormat, texturePixelDataType,
                                   {width, height, depth}, directDestination, internalBpp,
                                   destinationRowStride, destinationImageStride, internalBytes)) {
    ...
    void TexImage2D_State(GLenum target, GLint level, GLint internalformat, GLsizei width, GLsizei height, GLint border,
...
#ifdef MOBILEPZ_V1_CANDIDATE
            if (level == textureObject->GetLevelRange().x()) {
                ::MobilePZ::V1::RecordNullAllocation(textureObject->GetLifetimeId());
            }
#endif
#ifdef MOBILEPZ_PZF9_MODEL_TEXTURE_UPLOAD_PROVENANCE
            if (level == textureObject->GetLevelRange().x()) {
                ::MobilePZ::PZF9::RecordClientUpload(
                    textureObject->GetLifetimeId(), textureObject->GetExternalIndex(),
                    ::MobilePZ::PZF9::UploadKind::NullImage2D, level, 0, 0, width, height,
                    static_cast<Uint>(textureInternalFormat), format, type, pixelUnpackBufferObject != nullptr,
                    {}, {});
            }
#endif
#ifdef MOBILEPZ_PZF13_NULL_TEXTURE_PRODUCER_LINEAGE
            if (level == textureObject->GetLevelRange().x()) {
                ::MobilePZ::PZF13::RecordNullAllocation(
                    textureObject->GetLifetimeId(), textureObject->GetExternalIndex(), width, height);
            }
#endif
            return;
        }
        ...
        if (!isProxy &&
            TryDirectTextureUnpack(PZOptLab::Optimization::Perf022B, originalPixels, unpackParams,
                                   textureInternalFormat, textureInputFormat, texturePixelDataType,
                                   {width, height, 1}, directDestination, internalBpp,
                                   destinationRowStride, destinationImageStride, internalBytes,
                                   directInstrumentationCompatible)) {
            textureMipmapObject->MarkStorageDirty(textureUploadTarget, level, true);
            MaybeAutoGenerateMipmap(target, textureObject, false, level);
...
#ifdef MOBILEPZ_PZF9_MODEL_TEXTURE_UPLOAD_PROVENANCE
        const auto pzf9Source = ::MobilePZ::PZF9::AnalyzeUnpackSource(
            originalPixels, width, height, 1, inputBpp,
            unpackParams.Alignment, unpackParams.RowLength, unpackParams.ImageHeight,
            unpackParams.SkipPixels, unpackParams.SkipRows, unpackParams.SkipImages);
#endif
        processedPixels = MG_Util::PixelStoreProcessor::ProcessTexturePixelsDataUnpack(
        ...
        if (level == textureObject->GetLevelRange().x()) {
            ::MobilePZ::PZF9::RecordClientUpload(
                textureObject->GetLifetimeId(), textureObject->GetExternalIndex(),
                ::MobilePZ::PZF9::UploadKind::Image2D, level, 0, 0, width, height,
                static_cast<Uint>(textureInternalFormat), format, type, pixelUnpackBufferObject != nullptr,
                pzf9Source, ::MobilePZ::PZF9::AnalyzeBytes(processedPixels, imageSize));
        }
#endif
    ...
    void TexImage1D_State(GLenum target, GLint level, GLint internalFormat, GLsizei width, GLint border, GLenum format,
        ...
        if (!isProxy &&
            TryDirectTextureUnpack(PZOptLab::Optimization::Perf022B, originalPixels, unpackParams,
                                   textureInternalFormat, textureInputFormat, texturePixelDataType,
                                   {width, 1, 1}, directDestination, internalBpp, internalBytes,
...
#define GET_SRC_INTERNAL_FORMAT(AttachmentType)                                                                        \
    ...
    // BC/ETC codec here to decode the incoming blocks with. PERF-004R is the exception: when
    // enabled, the backend can forward an S3TC blob verbatim to a driver that advertises the
    // exact format, so the complete compressed level must be marked dirty after this patch.
    void CompressedTexSubImage2D_State(GLenum target, GLint level, GLint xoffset, GLint yoffset, GLsizei width,
        ...
        textureMipmapObject->SetMipmapCompressedImage(textureUploadTarget, static_cast<Uint>(level), format,
                                                      blob.data(), blobSize);
        if (PZOptLab::Enabled(PZOptLab::Optimization::Perf004R)) {
            textureMipmapObject->MarkStorageDirty(textureUploadTarget, static_cast<Uint>(level), true);
        }
    }
    ...
    void TextureSubImage2D(GLuint texture, GLint level, GLint xoffset, GLint yoffset, GLsizei width, GLsizei height,
        ...
        const SizeT srcRowSize = static_cast<SizeT>(width) * internalBpp;
        const SizeT destRowSize = static_cast<SizeT>(texelSize.x()) * internalBpp;
        const SizeT regionBytes = srcRowSize * static_cast<SizeT>(height);
        const SizeT fullLevelBytes = destRowSize * static_cast<SizeT>(texelSize.y());
        ...
        Uint8* directDestination =
            destData ? destData + static_cast<SizeT>(yoffset) * destRowSize +
                           static_cast<SizeT>(xoffset) * internalBpp
                     : nullptr;
        const auto markDirtyAfterDsaSubImage = [&] {
            if (PZOptLab::Enabled(PZOptLab::Optimization::Perf022E)) {
                PZOptLab::RecordPath(PZOptLab::Optimization::Perf022E,
                                     ProofByteCount(fullLevelBytes), ProofByteCount(regionBytes));
                textureMipmapObject->MarkStorageDirtyRegion(textureUploadTarget, level,
        ...
        if (TryDirectTextureUnpack(PZOptLab::Optimization::Perf022A, originalPixels, unpackParams,
                                   textureObject->GetFormat(), textureInputFormat, texturePixelDataType,
                                   {width, height, 1}, directDestination, internalBpp, destRowSize,
                                   fullLevelBytes, regionBytes)) {
            markDirtyAfterDsaSubImage();
#ifdef MOBILEPZ_PZF13_NULL_TEXTURE_PRODUCER_LINEAGE
            ::MobilePZ::PZF13::RecordWrite(textureObject->GetLifetimeId(), textureObject->GetExternalIndex(),
                                           0, -1, ::MobilePZ::PZF13::WriteKind::DirectUpload);
#endif
            return;
        }
        ...
        SizeT inputSize = 0;
        void* processedPixels = MG_Util::PixelStoreProcessor::ProcessTexturePixelsDataUnpack(
            originalPixels, unpackParams, textureObject->GetFormat(), textureInputFormat,
            texturePixelDataType, {width, height, 1}, false, inputSize);
        if (!processedPixels || inputSize == 0) {
            if (processedPixels) free(processedPixels);
            return;
        }
...
#ifdef MOBILEPZ_PZF13_NULL_TEXTURE_PRODUCER_LINEAGE
        if (textureObject && (access == GL_WRITE_ONLY || access == GL_READ_WRITE)) {
            ::MobilePZ::PZF13::RecordImageWriteBind(textureObject->GetLifetimeId(),
