...
namespace MobileGL::MG_Backend::DirectGLES {
    ...
    Bool MakeCurrent();
    Bool ReleaseCurrent();
    // Detaches and destroys only the active native EGLSurface. The native
    // display, config and GLES context stay alive so Android can leave the app
    // without a window and attach a replacement window on resume.
    Bool ReleaseSurface();
